...
class OrderService:
    ...
    def create_order(self, checkout_data):
        try:
            ...
            response = requests.post(
                f"http://127.0.0.1:8081/create-order", json=checkout_data
            )
            ...
            self.logger.error(
                "Error during order creation: %s", response_json["message"]
            )
        ...
        try:
            ...
            if transaction_data["result"] == "success":
                reservation_response = requests.get(
                    f"http://127.0.0.1:8081/process-paid-orders/refund-check?user_id={user_id}&order_id={order_id}"
                )
                ...
                if reservation_response_json["success"]:
                    ...
                    return (
                        f"/payment-success?user_id={user_id}&order_id={order_id}",
                        302,
                    )

                self.logger.error(
                    "Error in processing order: %s",
                    reservation_response_json["message"],
                )

                return (
                    url_for(
                        "refund_page", message=reservation_response_json["message"]
                    ),
                    302,
                )
            else:
    ...
    def process_delivery(self, user_id,order_id ):
        response = requests.post(
            f"http://127.0.0.1:8081/process-paid-orders/delivery?user_id={user_id}&order_id={order_id}"
        )

        if response.status_code != 200:
            self.logger.error("Error in process_delivery: %s", response.text)
            return False, {"message": "Error in processing delivery"}

        response_json = response.json()

        if not response_json["success"]:
            self.logger.error("Error in process_delivery: %s", response_json["message"])
            return False, {"message": response_json["message"]}
